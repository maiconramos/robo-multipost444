...
export async function GET(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    return publicApiResponse({ post: serializePost(post) }, authResult);
  } catch (error) {
    console.error("GET /api/public/v1/posts/[postId] error:", error);
    return publicApiResponse(
...
export async function PUT(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    const existing = await prisma.post.findFirst({
      where: { id: postId, workspaceId },
      include: { cronJob: true, mediaItems: true },
    });
    ...
    const hasScheduledForChange = Object.prototype.hasOwnProperty.call(
      parsed,
      "scheduledFor"
    );
      ...
      if (parsed.platforms && normalizedPlatforms) {
        ...
        await tx.platformPost.createMany({
          data: normalizedPlatforms.map((entry) => ({
            ...
            platformData: entry.platformSpecificData as
    ...
    console.error("PUT /api/public/v1/posts/[postId] error:", error);
    return publicApiResponse(
      { error: "Internal server error" },
      authResult,
      { status: 500 }
...
export async function DELETE(
  ...
  try {
    ...
    return publicApiResponse({ success: true }, authResult);
  } catch (error) {
    console.error("DELETE /api/public/v1/posts/[postId] error:", error);
    return publicApiResponse(
      { error: "Internal server error" },
      authResult,
      { status: 500 }
