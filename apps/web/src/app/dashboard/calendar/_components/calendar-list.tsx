...
export function CalendarList({
  ...
  const formatDayHeader = (date: Date) => {
    ...
    return formatWithLocale(date, "EEEE, MMMM d", locale);
  };
    ...
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        ...
        <p className="mt-4 text-sm font-medium text-muted-foreground">
          {t("No posts scheduled for {monthYear}", {
            monthYear: formatWithLocale(currentDate, "MMMM yyyy", locale),
          })}
        </p>
        <p className="mt-1 text-xs text-muted-foreground/70">
          {t("Create a post to see it here")}
        </p>
      </div>
    );
  }
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-2 flex items-center gap-3">
                    ...
                    <span className="flex items-center gap-1 text-xs text-muted-foreground">
                      ...
                      {formatWithLocale(parseISO(post.scheduledFor!), "h:mm a", locale)}
                    </span>
