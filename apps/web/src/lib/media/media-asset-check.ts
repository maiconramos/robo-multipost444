...
function isContentTypeCompatible(params: {
  ...
  if (params.declaredType === "image" && !normalizedActual.startsWith("image/")) {
    return false;
  }

  if (params.declaredType === "video" && !normalizedActual.startsWith("video/")) {
    return false;
  }

  const declared = normalizeMimeType(params.declaredContentType);
  if (declared && declared !== normalizedActual) {
    return false;
  }
...
async function inspectByFetch(url: string): Promise<CheckedAsset> {
  ...
  if (!rangeResponse.ok) {
    throw new Error(
      `Media URL is not accessible (HTTP ${rangeResponse.status}).`,
    );
  }
...
async function inspectByVercelBlob(params: {
  url: string;
  key?: string | null;
}): Promise<CheckedAsset | null> {
  const key = params.key && params.key.length > 0 ? params.key : params.url;

  try {
    const metadata = await head(key);
    return {
      contentType: normalizeMimeType(metadata.contentType),
...
export async function validateMediaAssetForPublishing(
  media: MediaReference,
): Promise<void> {
  let checked: CheckedAsset | null = null;

  if (media.provider === "VERCEL_BLOB") {
    checked = await inspectByVercelBlob({
      url: media.url,
      key: media.key,
    });
  }
  ...
  if (!checked) {
    checked = await inspectByFetch(media.url);
  }
...
export async function validatePostMediaAssetsForPublishing(
